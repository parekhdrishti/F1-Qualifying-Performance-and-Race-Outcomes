...
print(f"Linear Regession Coefficient (slope): {slope:.4f}")
...
plt.show()
# The scatter plot shows a clear upward trend, meaning better qualifying positions generally
# lead to better race finishes, although there is some variability
...
# Interpretation
# The distribution of position changes is centered around zero, indicating that most drivers
# finish close to their starting positions. Large gains or losses are less frequent,
# reinforcing the idea that starting position is an important factor in race outcomes.
...
plt.show()
# Most drivers finish close to their starting position, as shown by the concentration around
# zero. Large gains or losses are less common.
...
plt.show()
# The probability of finishing on the podium decreases sharply as starting position worsens,
# highlighting the importance of qualifying performance.
# This analysis shows that qualifying performance is a strong predictor of race outcomes in
# Formula 1. Drivers who start in higher grid positions are significantly more likely to finish
# in top positions, especially on the podium. While some variability exists due to race
# conditions and strategy, the overall trend confirms that starting position plays a crucial
# role in determining race success.
